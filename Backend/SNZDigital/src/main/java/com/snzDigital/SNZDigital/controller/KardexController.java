...
public class KardexController {
    ...
    @PostMapping("/create")
    public ResponseEntity<KardexResponse> createKardex(@RequestBody KardexEntity kardexEntity) {
        KardexEntity createdKardex = kardexService.createKardex(kardexEntity);
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(new KardexResponse(createdKardex.getKardexId(), "Kardex creado con éxito", true));
    }

    @PutMapping("/update/{id}")
    public ResponseEntity<KardexResponse> updateKardex(@PathVariable Integer id, @RequestBody KardexEntity kardexEntity) {
        KardexEntity updatedKardex = kardexService.updateKardex(id, kardexEntity);
